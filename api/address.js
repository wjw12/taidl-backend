...
module.exports = async (req, res) => {
  switch(req.method) {
        ...
        case 'GET': // get user ID of an address, returns empty string if not in-network
            ...
            if (address < 3) { res.status(400).send("invalid address"); return; }
            ...
            collection.findOne(q, (err, result) => {
                if (err) {
                    ...
                    if (result.isAnonymous) {
                        ...
                        res.send(200).send(result.userId);
                    }
                }
            })
            return;
        case 'POST': // set anonymous property
            const {body} = req;
            if (!body || !body.address || !body.isAnonymous) { res.status(400).send("incomplete data"); return; }
            ...
            collection.findOne(q, (err, result) => {
                if (err) {
                    ...
                    if (result.isAnonymous != body.isAnonymous) {
                        collection.updataOne(q, {$set: {isAnonymous: body.isAnonymous}}, (err, result) => {
